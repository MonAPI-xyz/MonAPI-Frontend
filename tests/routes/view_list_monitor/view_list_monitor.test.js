...
describe('Test API Monitors sites', () => {
	...
	test('there is a monitor in list', async () => {
		...
		await waitFor(() => {
			expect(screen.getAllByText('Ferdi')).toBeDefined()
	...
	test('last checked', async () => {
		...
		await waitFor(() => {
			expect(screen.getAllByText('Test API Monitor')).toBeDefined()
			expect(screen.getByText('https://api-staging.monapi.xyz')).toBeDefined()
			expect(screen.getByText('0.0%')).toBeDefined()
			expect(screen.getByText('20 ms')).toBeDefined()
			expect(screen.getByText(`Last checked: ${last_checked}`)).toBeDefined()
			expect(getCurrentUrl()).toBe('/');
		})
	})
	...
	test('indicator bar color', async () => {
		...
		await waitFor(() => {
			expect(screen.getByText('100% Success rate')).toBeDefined();
			expect(screen.getByText('1-99% Success rate')).toBeDefined();
			expect(screen.getByText('0% Success rate')).toBeDefined();
	...
	test('if there is no monitor in list', async () => {
		...
		await waitFor(() => {
			expect(screen.getByText('There is no monitor. You can click green button "Create New" in the middle right side')).toBeDefined()
			expect(getCurrentUrl()).toBe('/');
		})
	})
